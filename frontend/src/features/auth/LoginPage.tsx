...
export default function LoginPage() {
  ...
  const onSubmit = (data: LoginFormValues) => {
    login(data, {
      onSuccess: ({ data: res }) => {
        if (res?.user) {
          const dest = from && !from.startsWith("/auth") ? from : DASH[res.user.role]
  ...
  const handleResendForUnverified = () => {
    ...
    resendOtp(
      ...
      {
        onSuccess: () => {
          toast.success("OTP sent", "Check your WhatsApp for the verification code")
        ...
        onError: (err: any) => {
          ...
          if (typeof err?.message === "string" && err.message.includes("already verified")) {
            toast.info("Account already verified", "Please sign in with your password.")
    ...
    <AuthLayout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1.5">Sign in to your ClinicFlow account</p>
        </div>
        ...
        {unverifiedMode ? (
          <div className="space-y-5">
            <div className="p-4 bg-clinic-blue-light border border-blue-100 rounded-2xl space-y-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-clinic-blue">Account not verified yet</p>
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                value={whatsappDraft}
                onChange={e => { setWhatsappDraft(e.target.value); setWhatsappError("") }}
                onKeyDown={e => e.key === "Enter" && handleResendForUnverified()}
              />
            ...
            <Button size="lg" className="w-full" onClick={handleResendForUnverified} disabled={resending}>
              {resending
                ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Sending…</>
                : <><RefreshCw className="h-4 w-4 mr-2" />Send verification code</>}
            </Button>
          ...
          <>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Link to="/auth/forgot-password" className="text-xs text-clinic-blue hover:underline">
                    Forgot password?
                  </Link>
                </div>
            ...
            <p className="text-center text-sm text-muted-foreground">
              ...
              <Link to="/auth/register" className="text-clinic-blue font-medium hover:underline">
                Create account
              </Link>
            </p>
          </>
